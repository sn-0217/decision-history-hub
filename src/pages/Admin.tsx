...
import { ArrowLeft, Edit3, Save, X, CheckCircle, XCircle, Clock, Trash2, Calendar, User, FileText, Code, Settings, LogOut, Server, Database, Power, MoreVertical, Activity } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <Button 
              ...
              className="gap-2 h-9 px-3 border-slate-200 hover:border-slate-300 bg-white/50 backdrop-blur-sm hover:bg-white/80 text-slate-700 hover:text-slate-900 transition-all duration-200 hover:scale-[1.02] shadow-sm hover:shadow-md"
            >
              <ArrowLeft className="w-4 h-4 stroke-[1.5]" />
              <span className="font-medium">Back</span>
            </Button>
            
            {/* Refined Environment Badge */}
            <EnvironmentBadge currentEnv={currentEnv} />
            
            {/* Enhanced User Avatar */}
            <div className="flex items-center justify-center bg-gradient-to-br from-indigo-500 via-purple-500 to-indigo-600 text-white w-9 h-9 rounded-full font-semibold text-sm shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-105 ring-2 ring-white/20">
              <span className="select-none">{username?.charAt(0).toUpperCase() || 'A'}</span>
            </div>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button 
                  variant="ghost" 
                  size="sm" 
                  className="h-9 w-9 p-0 hover:bg-slate-100/80 hover:scale-105 transition-all duration-200 rounded-lg border border-transparent hover:border-slate-200 shadow-sm hover:shadow-md"
                >
                  <MoreVertical className="w-4 h-4 text-slate-600" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-56 p-2 bg-white/95 backdrop-blur-xl border border-slate-200/60 shadow-xl rounded-xl">
                <div className="px-3 py-2 mb-2 rounded-lg bg-slate-50/50">
                  <p className="text-xs font-medium text-slate-600 uppercase tracking-wide">Signed in as</p>
                  <p className="text-sm font-semibold text-slate-900">{username || 'Administrator'}</p>
                </div>
                <DropdownMenuItem className="gap-3 cursor-pointer py-2.5 px-3 rounded-lg hover:bg-slate-50 transition-colors">
                  <Activity className="w-4 h-4 text-slate-500" />
                  <span className="font-medium">Recent Activity</span>
                </DropdownMenuItem>
                <DropdownMenuSeparator className="my-2 bg-slate-200/60" />
                <DropdownMenuItem 
                  className="gap-3 cursor-pointer py-2.5 px-3 rounded-lg text-red-600 hover:text-red-700 hover:bg-red-50 focus:text-red-700 focus:bg-red-50 transition-colors"
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className={`grid w-full ${isAdmin ? 'grid-cols-6' : 'grid-cols-3'} max-w-3xl`}>
            ...
            <TabsTrigger value="health" className="gap-2 bg-gradient-to-r from-blue-50 to-indigo-50 hover:from-blue-100 hover:to-indigo-100 border-blue-200 text-blue-700 font-medium shadow-sm hover:shadow-md transition-all duration-200 hover:scale-105">
